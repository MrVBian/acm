...
int x[MAXN];//解集
bool l[MAXN];//标记是否是不确定的变元
inline int gcd(int a,int b){int t;while(b){t=b;b=a%b;a=t;}return a;}
...
int Gauss(int equ,int var)
{
    ...
    if (k < var){
        ...
        for (i = k - 1; i >= 0; i--)
        {
            ...
            for (j = 0; j < var; j++)
            {
                if (a[i][j] != 0 && l[j]) free_x_num++, free_index = j;
            }
